...
def draw_style_mixing_figure(png, Gs, w, h, src_seeds, dst_seeds, style_ranges, src_dlatents_dir, dst_dlatents_dir):
    # print(png)
    ...
    src_dlatents =  np.load(
        os.path.join(
            'latent_representations_black',
            src_dlatents_dir + '.npy'
        )
    ).reshape(1,18,512) # [seed, layer, component]


    dst_dlatents = np.load(
        os.path.join(
            'latent_representations_black',
            dst_dlatents_dir + '.npy'
        )
    ).reshape(1, 18, 512)  # [seed, layer, componentDSC_5980_01.npy]
...
def main():
    ...
    generate_num = 20
    tflib.init_tf()
    os.makedirs('./results_black', exist_ok=True)
    ...
    for i in range(0, generate_num):
        try:
            src2 = str(random.randint(0, total_image - 1))
            src1 = str(random.randint(0, total_image - 1))
            if src2 == src1:
                src1 = str(random.randint(0, total_image - 1))
            ...
            draw_style_mixing_figure(
                ...
                src_dlatents_dir = src1,
                dst_dlatents_dir = src2
            )
        except:
            pass
#----------------------------------------------------------------------------
